<x-splade-component is="dropdown" dusk="select-rows-dropdown" close-on-click>
    <x-slot:trigger>
        <input type="checkbox"
            class="rounded border-gray-300 text-primary-500 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:opacity-50"
    ...
    <div class="mt-2 min-w-max rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
        <div class="flex flex-col">
            <button
                ...
                {{ __('Seleccionar todas las filas de la página') }} ({{ $table->totalOnThisPage() }})
            </button>
            ...
            @if ($showPaginator())
                <button
                    ...
                    {{ __('Seleccionar todos las filas') }} ({{ $table->totalOnAllPages() }})
                </button>
            @endif
