@if ($table->hasExports() || $table->hasBulkActions())
    <div class="flex flex-col gap-y-4 shadow-sm my-4 py-5 pb-2 px-3 border rounded-md"
        v-if="table.hasSelectedItems || @js($table->hasExports())">
        @include('splade::table.bulk-actions-exports')
    </div>
@endif

<div class="flex flex-row sm:justify-end mb-3 sm:px-0 -mr-2 sm:-mr-3">
    ...
    <button v-show="@js($canResetTable()) || table.columnsAreToggled || table.hasForcedVisibleSearchInputs" type="button"
        ...
        <span class="ml-2 hidden sm:inline">{{ __('Limpiar') }}</span>
    </button>
