                ...
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        ...
                        @can('viewAny', \App\Models\User::class)
                            <x-nav-link :href="route('profesors.show')" :active="request()->routeIs('profesors.*')">
                                {{ __('Profesors') }}
                            </x-nav-link>
                        @endcan
                        
                        @can('viewAny', \App\Models\Category::class)
                            <x-nav-link :href="route('categories.show')" :active="request()->routeIs('categories.*')">
                                {{ __('Category') }}
                            </x-nav-link>
                        @endcan
                        
                        @can('viewAny', \App\Models\Departament::class)
                            <x-nav-link :href="route('departament.show')" :active="request()->routeIs('departament.*')">
                                {{ __('Departaments') }}
                            </x-nav-link>
                        @endcan
                        
                        @can('viewAny', \App\Models\Role::class)
                            <x-nav-link :href="route('role.show')" :active="request()->routeIs('role.*')">
                                {{ __('Roles') }}
                            </x-nav-link>
                        @endcan
                        
                        @can('viewAny', \App\Models\Asignature::class)
                            <x-nav-link :href="route('asignature.show')" :active="request()->routeIs('asignature.*')">
                                {{ __('Asignatures') }}
                            </x-nav-link>
                        @endcan
